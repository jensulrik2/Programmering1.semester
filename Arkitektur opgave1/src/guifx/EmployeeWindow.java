...
public class EmployeeWindow extends Stage {
	...
	private void initContent(GridPane pane) {
		...
		pane.add(lblEmployment, 0, 4);

		txfEmployment = new TextField();
		pane.add(txfEmployment, 0, 5);
	...
	private void initControls() {
		if (employee != null) {
			...
			txfEmployment.setText("" + employee.getEmploymentYear());
	...
	private void okAction() {
		...
		int employmentYear = Integer.parseInt(txfEmployment.getText().trim());
		if (name.length() == 0) {
			...
			if (wage < 0) {
				...
				if (employmentYear < 0){
					lblError.setText("Employment Year is not possible");
				}
			else {
